...
void Shop::addElements(int level)
{
	...
	case 1:
		...
		objects.push_back({ new Flashlight(game->getTexture(shop_Linterna), game,game->getPlayer()), 3, 0, 1, 50});
		break;
	case 2:
		objects.push_back({ new EnergyDrink(game->getTexture(shop_Soda2), game,game->getPlayer()), 0, 0, 3, 25 });
		objects.push_back({ new Banana(game->getTexture(shop_Banana), game,game->getPlayer()), 1, 0, 3, 15 });
		objects.push_back({ new Repel(game->getTexture(shop_Spray), game,game->getPlayer()), 0, 0, 2, 40 });
		objects.push_back({ new Lantern(game->getTexture(shop_Farol), game,game->getPlayer()), 0, 0, 1, 90 });
		break;
	case 3:
		objects.push_back({ new EnergyDrink(game->getTexture(shop_Soda2), game,game->getPlayer()), 0, 0, 3, 25 });
		...
		objects.push_back({ new Bike(game->getTexture(shop_Boots02), game,game->getPlayer()), 1, 0, 1, 99});
		break;
	default:
